#===================================================================================================================
# DATA Visualisation modules for each table in Apparent Consumption of Selected Foodstuffs, Australia, 2022-23
# Provides detailed consumption of foods, the amounts of Nutrients from those foods, based on supermarket sales data. 
# Data source is Apparent Consumption of Selected Foodstuffs, Australia, 2022-23
...
# #--Download datacube locally 
# daturl <- "//sasnasprd..../4316DO001_202223_ESTIMATES.xlsx"
# datapath <- "./datacube.xlsx"
# file.copy(from = daturl, to = datapath)
# 

#--Download datacube from the web 
daturl <- "https://www.abs.gov.au/statistics/health/health-conditions-and-risks/apparent-consumption-selected-foodstuffs-australia/2022-23/4316DO001_202223_ESTIMATES.xlsx"
datapath <- "./datacube.xlsx"
download.file(daturl,"datacube.xlsx", mode = "wb" )

#---Annual aggregates---
iA1.1 <- read_excel(datapath, sheet = 2, range = "A7:M148")  
iA2.1 <- read_excel(datapath, sheet = 4, range = "A8:G57")  
iA3.1 <- read_excel(datapath, sheet = 6, range = "A8:K27")  
iA3.2 <- read_excel(datapath, sheet = 7, range = "A6:AI68") 
iA4.1 <- read_excel(datapath, sheet = 8, range = "A8:K28")  
iA4.3 <- read_excel(datapath, sheet = 10, range = "A9:U87")  
iA4.4 <- read_excel(datapath, sheet = 11, range = "A9:U87")  
iA5.1 <- read_excel(datapath, sheet = 12, range = "A8:M148")
iA5.2 <- read_excel(datapath, sheet = 13, range = "A8:R148")
iA6.1 <- read_excel(datapath, sheet = 15, range = "A8:M148")
iA7.1 <- read_excel(datapath, sheet = 16, range = "A8:M148")
iA8.1 <- read_excel(datapath, sheet = 17, range = "A8:M148")
iA9.1 <- read_excel(datapath, sheet = 18, range = "A8:M148")
iA10.1 <- read_excel(datapath, sheet = 19, range = "A8:M148")
iA11.1 <- read_excel(datapath, sheet = 20, range = "A8:M148")
iA12.1 <- read_excel(datapath, sheet = 21, range = "A8:M148")
iA13.1 <- read_excel(datapath, sheet = 22, range = "A8:M148")
iA14.1 <- read_excel(datapath, sheet = 23, range = "A7:O189")


#--- Monthly aggregates---
iM1.2 <- read_excel(datapath, sheet = 3, range = "A7:BK147")
iM2.2 <- read_excel(datapath, sheet = 5, range = "A7:D2647")
iM3.2 <- read_excel(datapath, sheet = 7, range = "A6:AI68")
iM4.2 <- read_excel(datapath, sheet = 9, range = "A7:S69")
iM5.3 <- read_excel(datapath, sheet = 14, range = "A7:BK147") 
iM14.2 <- read_excel(datapath, sheet = 24, range = "A6:H10926") 
...
iA1.1names <- c("Class_level", "Code", "Label",
                "Tonnes,2018-19",  "Tonnes,2019-20", "Tonnes,2020-21", "Tonnes,2021-22", "Tonnes,2022-23",
                "g,2018-19",  "g,2019-20", "g,2020-21", "g,2021-22", "g,2022-23")

iA1.1pcNames <- c("Class_level", "Code", "Label", "2018-19",  "2019-20", "2020-21", "2021-22", "2022-23")
...
A1.1pc <- iA1.1 %>% 
  ...
  pivot_longer(cols = 4:8, names_to = "Year", values_to = "Val") %>% 
...
A1.1_ <- iA1.1 %>% 
  drop_na() %>% 
  setNames(iA1.1names) %>% 
  pivot_longer(cols = 4:13, names_to = "unityear", values_to = "Val") %>% 
...
A2.1 <- iA2.1 %>% 
  drop_na() %>% 
  rename(`2018-19` = `2018-19(c)`, `2019-20` = `2019-20(c)`, `2020-21` = `2020-21(c)`) %>% 
  pivot_longer(3:7, names_to = "Year", values_to = "Val") %>% 
...
iA3.1names <- c("Nutrient",
                "kJ_2018-19", "kJ_2019-20", "kJ_2020-21","kJ_2021-22",  "kJ_2022-23",                    
                "Pc_2018-19", "Pc_2019-20", "Pc_2020-21", "Pc_2021-22", "Pc_2022-23")

A3.1 <- iA3.1 %>% 
  drop_na() %>% 
  setNames(iA3.1names) %>% 
  pivot_longer(2:11, names_to = "Year", values_to = "Val") %>% 
...
iA4.1names <- c("ADG_group", "Grams,2018-19", "Grams,2019-20",  "Grams,2020-21", "Grams,2021-22", "Grams,2022-23",
                "Serves,2018-19", "Serves,2019-20",  "Serves,2020-21", "Serves,2021-22", "Serves,2022-23")
...
A4.1 <- iA4.1 %>% 
  ...
  pivot_longer(3:12, names_to = "typ_yr", values_to = "Val") %>% 
  ...
  mutate(ADG_group = case_when(ADG_group == "Meats, poultry, fish, eggs, tofu, nuts and seeds and legumes/beans" ~ "Meats", 
                               ...
                               TRUE ~ ADG_group)) %>% 
  mutate(Disc_type = case_when(nrow < 7 ~ "Disc", 
                               nrow %in% 7:12 ~ "NonDisc", 
...
iA4.3names <- c("ADG_subgroup", "Grams,2018-19", "Grams,2019-20", "Grams,2020-21", "Grams,2021-22", "Grams,2022-23",         
                "% (g),2018-19", "% (g),2019-20", "% (g),2020-21", "% (g),2021-22",  "% (g),2022-23", 
                "Serves,2018-19", "Serves,2019-20", "Serves,2020-21", "Serves,2021-22", "Serves,2022-23",   
                "% (Serves),2018-19", "% (Serves),2019-20", "% (Serves),2020-21", "% (Serves),2021-22", "% (Serves),2022-23" )
...
A4.3_4 <- A4.3 %>% 
  ...
  pivot_longer(4:23, names_to = "names", values_to = "Val") %>% 
...
iA5.2names <- c("Class_level", "Code", "Label", 
                "kJ,2018-19",  "kJ,2019-20", "kJ,2020-21","kJ,2021-22", "kJ,2022-23",
                "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23")
...
A5.2 <- iA5.2 %>% 
  ...
  pivot_longer(4:13, names_to = "Year", values_to = "Val") %>% 
...
kJ <- c("Class_level", "Code", "Label", "kJ,2018-19",  "kJ,2019-20", "kJ,2020-21", "kJ,2021-22", "kJ,2022-23",
        "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23" )

grams <- c("Class_level", "Code", "Label", "g,2018-19",  "g,2019-20", "g,2020-21", "g,2021-22", "g,2022-23",
           "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23" )

mg <- c("Class_level", "Code", "Label", "g,2018-19",  "mg,2019-20", "mg,2020-21", "mg,2021-22", "mg,2022-23",
        "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23" )
...
iA14.1names <- c("Class_order", "Beverage_group", "Measure",  "Unit", "Unit_type", 
                 "Amt,2018-19",  "Amt,2019-20", "Amt,2020-21", "Amt,2021-22", "Amt,2022-23",
                 "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23")

A14.1 <- iA14.1 %>% 
  drop_na() %>% 
  setNames(iA14.1names) %>% 
  mutate_at(6:15,as.numeric) %>% 
  pivot_longer(6:15, names_to = "Year", values_to = "Val") %>% 
...
M1.2 <- iM1.2 %>% 
  drop_na() %>% 
  rename("Class_level" = "Classification level",   "Code" =  "Food group code", "Label" = "Food group(c)(d)" ) %>% 
  pivot_longer(cols = 4:63, names_to = "mth", values_to = "Val")%>% 
  mutate(Val = as.double(Val)) %>% 
...
M2.2 <- iM2.2 %>% 
  drop_na() %>% 
  rename(Date = `Month(b)`, Val = `Mean daily amount per capita(c)`) %>% 
  mutate(Type = "All_nutrients") %>% 
                        ...
                        sidebarPanel(
                          ...
                          conditionalPanel(
                            ...
                            radioButtons("Class1", "Classification level:", 
                                         choices = c("Major", "Sub-major", "Components of major" = "MajMin",  "Total", "Beverages", "Disc. total" = "Discretionary status" ), 
                          ...
                          conditionalPanel(
                            ...
                            checkboxGroupInput("finyr", "Years:" ,
                                               choices= c("2018-19", "2019-20", "2020-21", "2021-22", "2022-23"), 
                                               selected = c("2018-19", "2019-20", "2020-21", "2021-22", "2022-23"), inline = TRUE), 
                        ...
                        mainPanel(
                          ...
                          tags$div(class="header", checked=NA,
                                   tags$p("Source: ", tags$a(href="https://www.abs.gov.au/statistics/health/health-conditions-and-risks/apparent-consumption-selected-foodstuffs-australia/latest-release",
                                                             target="_blank", "ABS Apparent Consumption of Selected Foodstuffs, Australia, 2022-23")),
...
server <- function(input, output) {  
  ...
  df_Ausn_fy <- reactive({ A_Ausnut %>%
      ...
      filter(Val_pc == Percent()$Percent) %>%  
      filter(cLabel %in% Majgrp1()$Majgrp1 | cLabel %in% Mingrp1()$Mingrp1 | Label %in% Othgrp() | submajCode %in% MajMin()) %>% 
      group_by(Year) %>% 
      arrange(desc(Val)) %>% 
      mutate(a =1) %>%
      mutate(c = cumsum(a)) %>% 
  ...
  df_Ausn_mth <- reactive({ 
    ...
    M_Ausnut %>%
      ...
      # Filter to keep only the labels that are in df_Ausn_fy
  ...
  # Define the custom function to apply font settings (bar / col)
    apply_font_styles <- function(hc, showDataLabels) {
      hc %>%
        ...
        hc_legend(
          itemStyle = list(fontSize = '14px')  # Set the font size for the legend
        ) %>%
        hc_plotOptions(
          series = list(
            dataLabels = list(
