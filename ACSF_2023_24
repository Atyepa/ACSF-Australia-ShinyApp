#===================================================================================================================
# DATA Visualisation modules for each table in Apparent Consumption of Selected Foodstuffs, Australia, 2023-24
# Provides detailed consumption of foods, the amounts of Nutrients from those foods, based on supermarket sales data. 
# Data source is Apparent Consumption of Selected Foodstuffs, Australia, 2023-24
...
# # Delete the existing (old) datacube before updating 
file_to_delete <- "./datacube.xlsx"
...
#--Download datacube from the web 
daturl <- "https://www.abs.gov.au/statistics/health/health-conditions-and-risks/apparent-consumption-selected-foodstuffs-australia/2023-24/4316DO001_202324_ESTIMATES.xlsx"
datapath <- "./datacube.xlsx"
download.file(daturl,"datacube.xlsx", mode = "wb" )

#---Annual aggregates---
iA1.1 <- read_excel(datapath, sheet = 2, range = "A7:O146")  
iA2.1 <- read_excel(datapath, sheet = 4, range = "A8:H57")  
iA3.1 <- read_excel(datapath, sheet = 6, range = "A8:M27")  
iA3.2 <- read_excel(datapath, sheet = 7, range = "A6:AI80") 
iA4.1 <- read_excel(datapath, sheet = 8, range = "A8:M28")  
iA4.3 <- read_excel(datapath, sheet = 10, range = "A9:Y87")  
iA4.4 <- read_excel(datapath, sheet = 11, range = "A9:Y87")  
iA5.1 <- read_excel(datapath, sheet = 12, range = "A8:O146")
iA5.2 <- read_excel(datapath, sheet = 13, range = "A8:U146")
iA6.1 <- read_excel(datapath, sheet = 15, range = "A8:O145")
iA7.1 <- read_excel(datapath, sheet = 16, range = "A8:O146")
iA8.1 <- read_excel(datapath, sheet = 17, range = "A8:O146")
iA9.1 <- read_excel(datapath, sheet = 18, range = "A8:O146")
iA10.1 <- read_excel(datapath, sheet = 19, range = "A8:O146")
iA11.1 <- read_excel(datapath, sheet = 20, range = "A8:O146")
iA12.1 <- read_excel(datapath, sheet = 21, range = "A8:O146")
iA13.1 <- read_excel(datapath, sheet = 22, range = "A8:O146")
iA14.1 <- read_excel(datapath, sheet = 23, range = "A7:Q189")


#--- Monthly aggregates---
iM1.2 <- read_excel(datapath, sheet = 3, range = "A7:BW147")
iM2.2 <- read_excel(datapath, sheet = 5, range = "A7:D3175")
iM3.2 <- read_excel(datapath, sheet = 7, range = "A6:AI80")
iM4.2 <- read_excel(datapath, sheet = 9, range = "A7:S81")
iM5.3 <- read_excel(datapath, sheet = 14, range = "A7:BW147") 
iM14.2 <- read_excel(datapath, sheet = 24, range = "A6:H13110") 

#--- AUSNUT short labels
shortlabel <- read.xlsx("https://github.com/Atyepa/AUSNUT/raw/master/AUSNUT_maj_subma_shortlabels.xlsx") %>% 
...
iA1.1names <- c("Class_level", "Code", "Label",
                "Tonnes,2018-19",  "Tonnes,2019-20", "Tonnes,2020-21", "Tonnes,2021-22", "Tonnes,2022-23", "Tonnes,2023-24",
                "g,2018-19",  "g,2019-20", "g,2020-21", "g,2021-22", "g,2022-23", "g,2023-24")

iA1.1pcNames <- c("Class_level", "Code", "Label", "2018-19",  "2019-20", "2020-21", "2021-22", "2022-23", "2023-24")
...
A1.1_ <- iA1.1 %>% 
  drop_na() %>% 
  setNames(iA1.1names) %>% 
  pivot_longer(cols = 4:15, names_to = "unityear", values_to = "Val") %>% 
...
iA2.1_Names <- c("Nutrient", "Unit", "2018-19",  "2019-20", "2020-21", "2021-22", "2022-23", "2023-24")

A2.1 <- iA2.1 %>% 
  drop_na() %>% 
  setNames(iA2.1_Names) %>% 
  pivot_longer(3:8, names_to = "Year", values_to = "Val") %>% 
  mutate(Type = "All_nutrients")

#-- Table 3.1: Energy from macronutrients, mean daily kJ per capita and proportion of total energy
iA3.1names <- c("Nutrient",
                "kJ_2018-19", "kJ_2019-20", "kJ_2020-21","kJ_2021-22",  "kJ_2022-23", "kJ_2023-24",
                "Pc_2018-19", "Pc_2019-20", "Pc_2020-21", "Pc_2021-22", "Pc_2022-23", "Pc_2023-24")

A3.1 <- iA3.1 %>% 
  drop_na() %>% 
  setNames(iA3.1names) %>% 
  pivot_longer(2:13, names_to = "Year", values_to = "Val") %>% 
...
iA4.1names <- c("ADG_group", "Grams,2018-19", "Grams,2019-20",  "Grams,2020-21", "Grams,2021-22", "Grams,2022-23", "Grams,2023-24",
                "Serves,2018-19", "Serves,2019-20",  "Serves,2020-21", "Serves,2021-22", "Serves,2022-23", "Serves,2023-24")
...
A4.1 <- iA4.1 %>% 
  ...
  pivot_longer(3:14, names_to = "typ_yr", values_to = "Val") %>% 
...
iA4.3names <- c("ADG_subgroup", "Grams,2018-19", "Grams,2019-20", "Grams,2020-21", "Grams,2021-22", "Grams,2022-23", "Grams,2023-24",
                "% (g),2018-19", "% (g),2019-20", "% (g),2020-21", "% (g),2021-22",  "% (g),2022-23", "% (g),2023-24",  
                "Serves,2018-19", "Serves,2019-20", "Serves,2020-21", "Serves,2021-22", "Serves,2022-23", "Serves,2023-24",   
                "% (Serves),2018-19", "% (Serves),2019-20", "% (Serves),2020-21", "% (Serves),2021-22", "% (Serves),2022-23", "% (Serves),2023-24" )
...
A4.3_4 <- A4.3 %>% 
  ...
  pivot_longer(4:27, names_to = "names", values_to = "Val") %>% 
...
iA5.2names <- c("Class_level", "Code", "Label", 
                "kJ,2018-19",  "kJ,2019-20", "kJ,2020-21","kJ,2021-22", "kJ,2022-23", "kJ,2023-24",
                "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23", "Pc,2023-24" )
...
A5.2 <- iA5.2 %>% 
  ...
  pivot_longer(4:15, names_to = "Year", values_to = "Val") %>% 
  separate(Year, into = c("Unit", "Year"), sep = ",", extra = "merge") %>% 
  mutate(Nutrient = "Disc. Energy")



#--Tables 5.1 to 13.1 uses a function

# Define fun
process_dataframe <- function(df, nutrient, nutr_colnames) {
  
  df %>% 
    drop_na() %>% 
    setNames(nutr_colnames) %>%  
    mutate_at(4:15, as.numeric) %>% 
    pivot_longer(4:15, names_to = "Year", values_to = "Val") %>% 
    separate(Year, into = c("Unit", "Year"), sep = ",", extra = "merge") %>% 
...
#Define colnames for function

# kJ Energy;  g Protein; g Carbohydrate; g Total fat; g Added sugars; g Free sugars; g Saturated fat, mg Sodium; g Dietary fibre
kJ <- c("Class_level", "Code", "Label", "kJ,2018-19",  "kJ,2019-20", "kJ,2020-21", "kJ,2021-22", "kJ,2022-23", "kJ,2023-24",
        "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23", "Pc,2023-24")

grams <- c("Class_level", "Code", "Label", "g,2018-19",  "g,2019-20", "g,2020-21", "g,2021-22", "g,2022-23", "g,2023-24",
           "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23", "Pc,2023-24")

mg <- c("Class_level", "Code", "Label", "g,2018-19",  "mg,2019-20", "mg,2020-21", "mg,2021-22", "mg,2022-23", "mg,2023-24",
        "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23", "Pc,2023-24" )

A5.1 <- process_dataframe(iA5.1, "Energy", kJ)
A6.1 <- process_dataframe(iA6.1, "Protein", grams)
A7.1 <- process_dataframe(iA7.1, "Carbohydrate", grams)
A8.1 <- process_dataframe(iA8.1, "Total fat", grams)
A9.1 <- process_dataframe(iA9.1, "Added sugars", grams)
A10.1 <- process_dataframe(iA10.1, "Free sugars", grams)
A11.1 <- process_dataframe(iA11.1, "Saturated fat", grams)
A12.1 <- process_dataframe(iA12.1, "Sodium", mg)
A13.1 <- process_dataframe(iA13.1, "Dietary fibre", grams)
...
iA14.1names <- c("Class_order", "Beverage_group", "Measure",  "Unit", "Unit_type", 
                 "Amt,2018-19",  "Amt,2019-20", "Amt,2020-21", "Amt,2021-22", "Amt,2022-23", "Amt,2023-24",
                 "Pc,2018-19",  "Pc,2019-20", "Pc,2020-21", "Pc,2021-22", "Pc,2022-23", "Pc,2023-24")
...
A14.1 <- iA14.1 %>% 
  ...
  pivot_longer(6:17, names_to = "Year", values_to = "Val") %>% 
...
M1.2 <- iM1.2 %>%
  drop_na() %>%
  rename("Class_level" = "Classification level", "Code" = "Food group code") %>%
  rename_with(~ ifelse(str_detect(., "^Food group"), "Label", .)) %>%
  pivot_longer(cols = 4:75, names_to = "mth", values_to = "Val") %>%
  mutate(Val = as.double(Val)) %>%
  mutate(Nutrient = "Weight", Unit = "g") %>%
  mutate(Date = str_sub(mth, 1, 8)) %>%
  mutate(Date = dmy(paste0("01 ", Date))) %>%
  select(Class_level, Code, Label, Date, Unit, Val, Nutrient)

# Table 2.2 - (nutrients monthly)
iM2.2$Unit <- gsub("ug", "µg", iM2.2$Unit)  # Fix microgram symbol

M2.2 <- iM2.2 %>% 
  drop_na() %>% 
  rename(Date = `Month`) %>% 
  rename_with(~ ifelse(str_detect(., "^Mean daily amount per capita"), "Val", .)) %>%
  mutate(Type = "All_nutrients") %>% 
...
A_Ausnut01 <- A1.1 %>% 
  ...
  left_join(shortlabel, by = "Code") %>%  
  select(Class_level, Code, short_label, everything()) %>% 
  mutate(short_label = case_when(is.na(short_label) ~ Label, TRUE ~ short_label)) %>% 
  select(-Label) %>% 
  rename(Label = short_label) %>% 
  ...
  mutate(Label = str_remove_all(Label, ("\\(f\\)"))) %>% 
  ...
  mutate(two_dig = substr(Code, 1,2)) 
...
A_Ausnut02 <- A_Ausnut01 %>% 
  bind_rows(majmin)


# Add Two-dig parent for three-dig:
two_dig_label <- A_Ausnut02 %>% 
  select(Code, Label) %>% 
  mutate(Coden = as.integer(Code)) %>% 
  filter(Coden < 100) %>% 
  mutate(submajCode = paste0(Code, " ", Label)) %>% 
  select(-c(Coden, Label)) %>% 
  rename(Codec = Code) %>% 
  distinct()

A_Ausnut <- A_Ausnut02 %>% 
  left_join(two_dig_label, by = c("two_dig" = "Codec")) %>% 
  mutate(submajCode = case_when(!is.na(cLabel) ~ NA_character_, TRUE ~ submajCode)) %>% 
  mutate(Class_level = case_when(Class_level %in% c("Discretionary status", "Beverages", "Total") ~ "Other", 
                                 TRUE ~ Class_level))

# Dummy rows to filter on for percent & absolute change
A_Ausnut_dmmypc <- A_Ausnut %>% 
  filter(Val_pc == "Value") %>% 
  mutate(Val_pc = "pc_change")

A_Ausnut_dmmyg <- A_Ausnut %>% 
  filter(Val_pc == "Value") %>% 
  mutate(Val_pc = "g_change")

A_Ausnut <- A_Ausnut %>% 
  bind_rows(A_Ausnut_dmmypc, 
            A_Ausnut_dmmyg)
...
A_nut <- A2.1 %>% 
  ...
  mutate(Nutrient = str_remove_all(Nutrient, ("\\(c\\)"))) %>%
...
M_Ausnut01 <- M1.2 %>%
  bind_rows(M5.3) %>%
  mutate(Nutrient = str_remove(Nutrient, "\\(e\\)")) %>%
  mutate(Nutrient = str_remove(Nutrient, "\\(g\\)")) %>%
  mutate(Nutrient = str_remove(Nutrient, "\\(d\\)")) %>%
  mutate(Label = str_remove(Label, "\\(f\\)")) %>%
  left_join(shortlabel, by = "Code") %>%
  mutate(Label = coalesce(short_label, Label)) %>%
  select(Class_level, Code, Label, everything()) %>%
  mutate(cLabel = case_when(Code != "n.a." ~ paste0(Code, " ", Label),
                            Code == "n.a." ~ Label)) %>% 
  mutate(two_dig = case_when(Code != "n.a." ~ substr(Code, 1, 2),
                             Code == "n.a." ~ "n.a."))
...
M_Ausnut02 <- M_Ausnut01 %>% 
  bind_rows(majmin)

# Add Two-dig parent for three-dig:
two_dig_label <- M_Ausnut02 %>% 
  select(Code, Label) %>% 
  mutate(Coden = as.integer(Code)) %>% 
  filter(Coden < 100) %>% 
  mutate(submajCode = paste0(Code, " ", Label)) %>% 
...
bev_lvl <- c("Cordials", 	"Cordials - intense sweetened", 	"Cordials - sugar sweetened", 	"Electrolyte drinks", "Electrolyte drinks - intense sweetened",
             ...
             "Water", 	"Soft drinks", "Soft drinks - intense sweetened", 
    ...
    .paginate_button:active {
        ...
        border-color: black!important;
                  ...
                  "))),
                        ...
                        headerPanel("ACSF, 2023-24"),
                        
                        sidebarPanel(
                          radioButtons ("choosetype", "Explore apparent consumption amounts from:",
                                        choices = c("Tables 1, 5-13: g, kJ and selected nutrients by AUSNUT" = "AUSNUT", 
                          ...
                          ##@ Nested condition to cover AUSNUT for both finyr && month: 
                          
                          conditionalPanel(
                            condition = "input.choosetype == 'AUSNUT'",
                            
                            radioButtons("Class1", "Classification level:", 
                                         choices = c("Major", "Sub-major", "Sub-major within Major" = "MajMin",  
                                                     "Other grouping" = "Other"), 
                                         selected = "Other", inline = TRUE),
                          
                         # nested inside input$AUSNUT
                          conditionalPanel(
                            condition = "input.Class1 == 'Other'",
                          ...
                          conditionalPanel(
                            condition = "input.Class1 == 'Major' || input.Class1 == 'Sub-major'", 
                              numericInput("Num", "Max number of groups to display", value = 5)),
                          
                          conditionalPanel(
                            condition = "input.Class1 == 'MajMin'", 
                            
                           pickerInput("MajMin", "Components of major food groups:", choices = c(Two_dig),
                                       multiple = TRUE, options = list(`actions-box` = TRUE)))),
                          
                          radioButtons("colbar", "Plot to column or bar", choices = c("bar", "column"), inline = TRUE, selected = "column"),
                          checkboxInput("showDataLabels", "Show Data Labels", value = FALSE),
                            
                            # cond = input.choosetime == 'finyr'
                          conditionalPanel(
                            ...
                            checkboxGroupInput("finyr", "Years:" ,
                                               ...
                                               selected = c("2018-19", "2019-20", "2020-21", "2021-22", "2022-23", "2023-24"), inline = TRUE), 
                            ...
                            radioButtons("Percent", "Value, percent or percent change",
                                         choices = c("Value", "% of value" = "%", "Absolute change" = "g_change", "% change" = "pc_change"), 
                                         selected = "Value", inline = TRUE),
                            
                            radioButtons("Ausnstack", "Stacked/normal barplot:", choices = c("Stacked", "Normal"), selected = c("Normal"), inline = TRUE)),
                          
                          
                          # cond = input.choosetime == 'month'
                          conditionalPanel(
                            ...
                            sliderInput("dateRange","Start-end months:",
                                        ...
                                        step = 5,
                                        timeFormat="%b %Y"),
                            radioButtons("gkJ", "Select weight (g) or energy (kJ):", choices = c("g", "kJ"), selected = "g", inline = TRUE)),      
                        ...
                        mainPanel(
                          ...
                          tabsetPanel(type = "tabs",
                                      tabPanel("Graph", highchartOutput("hcontainer",height = "755px")),
                          ...
                          tags$div(class="header", checked=NA,
                                   tags$p("Source: ", tags$a(href="https://www.abs.gov.au/statistics/health/health-conditions-and-risks/apparent-consumption-selected-foodstuffs-australia/latest-release",
                                                             target="_blank", "ABS Apparent Consumption of Selected Foodstuffs, Australia, 2023-24")),
...
server <- function(input, output) {  
 ...
 Units3a <-  reactive({
  ...
  #---- Dataframes for plotting / tabulation----
   ...
   df_Ausn_fy <- reactive({
    # Step 1: Filter and count for the latest year
    latest_year_count <- A_Ausnut %>%
      filter(Year == "2023-24") %>%
      filter(Class_level %in% Class1()$Class1) %>%
      filter(Nutrient %in% NutA()$A_Nutrient) %>%
      filter(Val_pc == Percent()$Percent) %>%  
      filter(cLabel %in% Majgrp1()$Majgrp1 | cLabel %in% Mingrp1()$Mingrp1 |
               Label %in% Othgrp()$Othgrp | submajCode %in% MajMin()) %>% 
      arrange(desc(Val)) %>%
      mutate(a = 1) %>%
      mutate(c = cumsum(a)) %>%
      filter(c <= Num()$Num) %>%
    ...
    # Step 2: Join this count with the original dataset
    A_Ausnut %>%
      filter(Class_level %in% Class1()$Class1) %>%
      filter(Nutrient %in% NutA()$A_Nutrient) %>%
      filter(Val_pc == Percent()$Percent) %>%  
      filter(cLabel %in% Majgrp1()$Majgrp1 | cLabel %in% Mingrp1()$Mingrp1 |
               Label %in% Othgrp()$Othgrp | submajCode %in% MajMin()) %>% 
      left_join(latest_year_count, by = "Label") %>%
  ...
  df_Ausn_pc_chg <- reactive({
    df_Ausn_fy() %>% 
      filter(Year %in% c("2023-24", "2022-23", "2018-19")) %>%
      group_by(Label, Class_level, Unit, Nutrient) %>% 
      summarize(
        `1 year change` = round((Val[Year == "2023-24"] - Val[Year == "2022-23"]) / Val[Year == "2022-23"] * 100, 1),
        `5 year change` = round((Val[Year == "2023-24"] - Val[Year == "2018-19"]) / Val[Year == "2018-19"] * 100, 1),
        `1 year absolute change` = round(Val[Year == "2023-24"] - Val[Year == "2022-23"],1),
        `5 year absolute change` = round(Val[Year == "2023-24"] - Val[Year == "2018-19"],1)
      ) %>% 
      pivot_longer(
        cols = c(`1 year change`, `5 year change`, `1 year absolute change`, `5 year absolute change`),
  ...
  df_Ausn_mth <- reactive({ 
    # Get the unique labels from df_Ausn_fy
    fy_labels <- unique(df_Ausn_fy()$Label)
    
    M_Ausnut %>%
      filter(Unit == gkJ()$gkJ) %>%
      filter(Class_level %in% Class1()$Class1) %>% 
      filter(cLabel %in% Majgrp1()$Majgrp1 | cLabel %in% Mingrp1()$Mingrp1 | Label %in% Othgrp()$Othgrp | submajCode %in% MajMin()) %>% 
      group_by(Nutrient, cLabel) %>% 
      arrange(Date) %>%
      filter(Date >= input$dateRange[1]) %>%  
      filter(Date <= input$dateRange[2]) %>% 
      filter(Label %in% fy_labels) %>% 
      select(-cLabel)    
  })
  ...
  # Table for download
  df_ADGsub_out <- reactive({
    df_ADGsubgrp() %>%
      select(Year, Val, Unit_label, ADG_subgroup) %>%
  ...
  groupby <- reactive({
    if (input$Class1 == "Other" || input$Num < 3 ) {
      return("Label")
    } else {
      return("Year")
    }
  ...
  # Reactive object for x_axis in plot
  x_axis <- reactive({
    if (groupby() == "Label") {
      return("Year")
    } else {
  ...
  # Function to determine the number of unique values in Beverage_group
  num_unique_beverages <- reactive({
    length(unique(df_SSB_fy()$Beverage_group))
  })
  ...
  # Define the custom function to apply font settings (line)
    apply_font_styles_line <- function(hc, showDataLabels) {
      hc %>%
        hc_xAxis(
          title = list(style = list(fontSize = '18px')),
          labels = list(style = list(fontSize = '16px'))
    ...
    # 1) AUSNUT x year   (not stacked)
    if(input$choosetype == 'AUSNUT' &  input$choosetime == 'Finyr' & input$Ausnstack == "Normal") {   
      data <- df_Ausn_fy()
      
      if (x_axis() == "Year" && groupby() == "Label") {
        data <- data %>% 
          arrange(Year) %>% 
          ungroup()
      } else {
    ...
    # 2) AUSNUT x year (stacked)
    if(input$choosetype == 'AUSNUT' & input$choosetime == 'Finyr' & input$Ausnstack == "Stacked") {   
      data <- df_Ausn_fy() %>% 
          group_by(Label) %>% 
          arrange(Year) %>% 
          ungroup()
  ...
  # 1) AUSNUT x year  by percent change 
    if(input$choosetype == 'AUSNUT' & input$choosetime == 'Finyr' & input$Percent == 'pc_change') {   
      hc <- df_Ausn_pc_chg() %>% 
        filter(str_detect(change_type, "year change$")) %>%  # Filter to only include percentage changes
        ...
        hc_title(text = paste0("Annual and five-year percent change to 2023-24: ", input$A_Nutrient, " per capita - ", Uchg()$unit )) %>%
        hc_add_theme(hc_theme_economist()) %>% 
        hc_colors(abscol) %>% 
        hc_tooltip(crosshairs = TRUE, valueSuffix = paste0(" ","%")) %>% 
        apply_font_styles(input$showDataLabels)
    }
    
    # 1.1) AUSNUT x year by absolute change 
    if(input$choosetype == 'AUSNUT' & input$choosetime == 'Finyr' & input$Percent == 'g_change') {   
      hc <- df_Ausn_pc_chg() %>% 
        filter(str_detect(change_type, "absolute change$")) %>%  # Filter to only include absolute changes
        arrange(desc(change_value)) %>% 
        hchart(.,
               type = "bar",
               hcaes(x = Label, 
                     ...
                     group = change_type)) %>% 
        hc_xAxis(title = list(text = paste0("Selected food groups"))) %>%
        hc_yAxis(title = list(text = paste0("Change in ", U()$unit))) %>%
        hc_title(text = paste0("Annual and five-year change to 2023-24: ", input$A_Nutrient, " per capita - ",  U()$unit )) %>%
        hc_add_theme(hc_theme_economist()) %>% 
        hc_colors(abscol) %>% 
        hc_tooltip(crosshairs = TRUE, valueSuffix = paste0(U()$unit)) %>% 
        apply_font_styles(input$showDataLabels)
    ...
    if(input$choosetype == 'AUSNUT' &  input$choosetime == 'Month') {
      hc <- df_Ausn_mth() %>% 
        ...
        apply_font_styles_line(input$showDataLabels)
    }      
    ...
    # 7.1) SSB x finyr Val side-by-side + stacking  
      ...
      # Determine the hcaes mapping based on the number of unique values and stacking option
      hcaes_mapping <- if (num_unique_beverages() > 2 & input$graphstack2 == "Normal") {
        hcaes(x = Beverage_group, y = Val, group = Year)
      } else {
        hcaes(x = Year, y = Val, group = Beverage_group)
      }
      ...
      # Aggregate Val and order Beverage_group by descending total Val if using Beverage_group as x-axis
      if (num_unique_beverages() > 2 & input$graphstack2 == "Normal") {
        data <- data %>%
          arrange(desc(Val))
      }
    ...
    if(input$choosetype == 'SSB' &  input$choosetime == 'Finyr' & input$AmtPc == "Pc" ){
      ...
      # Determine the hcaes mapping based on the number of unique values and stacking option
      hcaes_mapping <- if (num_unique_beverages() > 2 & input$graphstack2 == "Normal") {
        hcaes(x = Beverage_group, y = Pc, group = Year)
      } else {
        hcaes(x = Year, y = Pc, group = Beverage_group)
      }
      ...
      hc <- data %>%
        hchart(.,
               ...
               hcaes_mapping) %>%
        hc_xAxis(title = list(text = paste0("Selected beverages"))) %>%
        hc_yAxis(title = list(text = paste0("Percent"))) %>%
        hc_title(text = paste0("Selected beverages - ", input$BevM)) %>%
        hc_add_theme(hc_theme_economist()) %>%
        hc_colors(abscol) %>%
        hc_tooltip(crosshairs = TRUE, valueSuffix = paste0("%")) %>%
  ...
  dt_SSB_fyPc <- reactive({df_SSB_fy() %>% 
      select(Unit, Beverage_group, Year, Pc) %>% 
      mutate(Unit = "%") %>% 
      arrange(Year) %>% 
      pivot_wider(1:2, names_from = Year, values_from = Pc)})
  ...
  output$table = DT::renderDataTable({
    ...
    if (input$choosetype == 'AUSNUT' &  input$choosetime == 'Finyr' & input$Percent == 'pc_change') {
      tab <- df_Ausn_pc_chg() %>% 
     rename('Percent change' = change_value) %>% 
      pivot_wider(names_from = change_type, values_from = 'Percent change')
    }
  ...
  # Table for download
  dltab <- reactive({
    tab <- switch(input$choosetype,
                  'AUSNUT' = switch(input$choosetime,
                                    'Finyr' = if (input$Percent == 'pc_change') {
                                      df_Ausn_pc_chg() %>% pivot_wider(names_from = change_type, values_from = percentage_change)
                                    } else {
                                      dt_Ausn_fy()
                                    },
                                    'Month' = dt_Ausn_mth()
                  ),
                  'NUT' = switch(input$choosetime,
                                 'Finyr' = dt_Nut_fy(),
                                 'Month' = dt_Nut_mth()
                  ),
                  'ADG' = switch(input$choosetime,
                                 'Finyr' = if (input$Subgrp == 'ADG_sub') {
                                   dt_ADGsubgrp()
                                 } else {
                                   dt_ADG_fy()
                                 },
  ...
  output$downloadTb <- downloadHandler(
    filename = function() { paste0("ACSF23-24, ", input$choosetime,", selected groups", ".xlsx") },
